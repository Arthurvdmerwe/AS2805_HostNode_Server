...
class ATMClient(Thread):
    ...
    def SendCWReversal(self, rrn):
        raw = ""
        raw += "<STX>DMXMD   td7W0       <FS>%s" % self.Atm_ID
        raw += "<FS>29<FS>" + rrn + "<FS>4089670000392726=17112011000017980000<FS>00012000<FS>00000200<FS>00004000<FS>VA5.00.07WV02.70.10 V04.00.19 0  0T  00 000     01800002K0200000000002K050000020000000000000000000000000000000000<FS>^E02E 5183<FS><ETX>"
        # raw.Append("<ETX>")
    ...
    def SendCWAuthorization(self, rrn):
        # Config
        raw = ""
...
while True:
    ...
    atm = ATMClient('S9218163')
    atm.Connect()
    atm.Read(30.0)
    atm.SendConfigRequest()
    #atm.SendCWReversal()
    atm.Read(90.0)
    atm.SendACK()
    atm.Read(30.0)
    ...
    atm.Connect()
    atm.Read(30.0)
    atm.SendCWAuthorization(str(rrn))
    #atm.SendCWReversal()
    atm.Read(90.0)
    atm.SendACK()
    atm.Read(30.0)
