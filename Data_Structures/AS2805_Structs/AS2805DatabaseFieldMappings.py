...
AS2805_to_DB = {"2": "p2_pan", "3": "p3_processing_code", "4": "p4_amount_tran", "5": "p5_amount_settlement",
                ...
                "90": "p90_original_data", "95": "p95_replacement_amounts", "97": "p97_amt_net_settle",
                "99": "p99_settle_id_code", "100": "p100_receving_id_code", "112": "p112_key_mng_data",
                "118": "p118_cash_no", "119": "p119_cash_amount", "128": "p128_mac_extended",
                "125": "p125_network_management_info"}
...
def Build_AS_Insert_Field_And_Values_for_Host_Node(iso, extra=None):
    """
    Builds an insert statement to save all the fields from an AS2805 message into the database.cuscal table
    ...
    #        log.debug('Bit %s of type %s with value = %s' % (v['bit'], v['type'], v['value']))
        ...
        try:
            ...
            print("DB Field Mapping does not exist %s" % e)
    ...
    sql = 'INSERT INTO host_cuscal (\n'
    sql += '  message,\n'
    sql += '  ' + field_list + ')\n'
    sql += 'VALUES (\n'
    sql += '  "%s",\n' % MySQLdb.escape_string((iso.getMTI()))
    sql += '  ' + value_list + ')'
...
def Get_AS_from_Database_Table(cur, tran_gid, message):

    cur.execute('SELECT * FROM host_cuscal where tran_gid = "%s"  AND message = "%s" limit 1' % (tran_gid, message))
    r = cur.fetchone()
    if r > 0:
        result_list = r
        resp_iso = AS2805()
        resp_iso.setMTI(result_list['message'])
        for item in result_list:

            if result_list[item] is not None:
                for k, v in AS2805_to_DB.items():
                    if v == item:
                        resp_iso.setBit(int(k), result_list[item])
        return resp_iso
    else:
        return None